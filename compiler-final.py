...
class Scanner:
    ...
    def consume(self, *expected_tokens):
        ...
        # if token is NUM or ID, it is an identifier and thus must return value of identifier also

        if token in expected_tokens:
            self.current_token = self.get_token()
            if token == Token.NUM or token == Token.ID:
                return token, value
            else:
                return token
            ...
            # not sure what the excpected token is, solved

class Token:
    # The following enumerates all tokens.

    """Extend the class Token by the reserved words read and write, by the symbols !=, * and / for inequality, 
    multiplication and division, and by number constants. You will have to add a regular expression for each of these 
    tokens and new token constants READ and WRITE. Use the existing 
    token constants for inequality, multiplication, division and numbers."""
    ...
    LPAR = 'LPAR'
    RPAR = 'RPAR'
    NUM = 'NUM'  # this is the number constants that need to be implemented
    ID = 'ID'
    ...
    token_regexp = [
        ...
        (THEN, 'then'),
        (WHILE, 'while'),
        (READ, 'read'),  # new not sure
        (WRITE, 'write'),  # new not sure
        (SEM, ';'),
        (BEC, ':='),
        (LESS, '<'),
        (EQ, '='),
        ...
        (NUM, '[0-9]+'),  # new not sure, think it should be * instead of +
        (ID, '[a-z]+')
...
class If_Else_AST:
    ...
    def __init__ (self, condition, then, if_else_then):
        self.condition = condition
        self.then = then
        self.if_else_then = if_else_then

    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
               repr(self.then) + ' else ' + repr(self.if_else_then) + 'end'

    def indented(self, level):
        return indent('If-Else', level) + \
               self.condition.indented(level + 1) + \
               self.then.indented(level + 1) + self.if_else_then.indented(level + 1)
...
class Comparison_AST:
    ...
    def false_code(self, label):
        ...
        #records the opposites of each label.
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        if_else_then = statements()
        scanner.consume(Token.END)
        return If_Else_AST(condition, then, if_else_then)
